...
class IntegrationRegistry:
    ...
    `get()` resolves a single, workspace-wide, always-available provider by
    name -- today only "local" is registered this way, since it needs no
    ...
    `IntegrationAccount` is one person's mailbox). It looks up that user's own
    connected account and hands back a *fresh* provider instance bound to it
    -- never a shared singleton, and never a fallback to the "local"/default
    provider (a caller that gets `None` back decides for itself what "not
    connected" means: context retrieval skips Gmail as a search source;
    `execute_action` uses the local provider instead).
    """

    def __init__(
        self, default: IntegrationProvider, *,
        integration_accounts: Repository[IntegrationAccount] | None = None,
        provider_factories: dict[str, Callable[[IntegrationAccount], IntegrationProvider]] | None = None,
    ) -> None:
    ...
    def register_factory(self, provider_name: str, factory: Callable[[IntegrationAccount], IntegrationProvider]) -> None:
    ...
    def resolve_for_user(self, workspace_id: str, user_id: str, *, provider_name: str) -> IntegrationProvider | None:
